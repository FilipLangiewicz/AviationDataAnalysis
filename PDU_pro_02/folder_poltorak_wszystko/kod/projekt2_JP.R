...
# 
# transposed_data %>% ggplot(aes(przedzialy,Value,fill=Company))+geom_col(position="dodge")+labs(title="Dupa",x="Przedziały",y="Ratio")
...
ggplot(data=binded,aes(Year,count,fill=UniqueCarrier)) +geom_col(position="dodge")+geom_line(data= blue_data,aes(x=Year,y=ratio*4800000,group=1),color="blue",size=2) + geom_line(data=red_data,aes(x=Year,y=ratio*4800000,group=1),color="red",size=2) +scale_y_continuous(sec.axis = sec_axis(~./4800000),name="dupa")

ggplot(data=binded,aes(Year,count,fill=UniqueCarrier)) +geom_col(position="dodge") +scale_fill_grey()+geom_line(data= blue_data,aes(x=Year,y=ratio*4800000,group=1),color="blue",size=2) + geom_line(data=red_data,aes(x=Year,y=ratio*4800000,group=1),color="red",size=2) +scale_y_continuous(sec.axis = sec_axis(~./4800000),name="dupa")+ theme_bw()

ggplot(data=binded,aes(Year,count,fill=UniqueCarrier)) +geom_col(position="dodge") +scale_fill_manual(values=viridis(10)[c(2,8)])+geom_line(data= blue_data,aes(x=Year,y=ratio*4800000,group=1),color="blue",size=2) + geom_line(data=red_data,aes(x=Year,y=ratio*4800000,group=1),color="red",size=2) +scale_y_continuous(sec.axis = sec_axis(~./4800000),name="dupa")+ theme_bw()

#finalny na godzine 22:30, testowy
ggplot(data=binded,aes(Year,count,fill=UniqueCarrier)) +geom_col(position="dodge") + scale_fill_manual(values=viridis(10)[c(2,8)])+geom_line(data= blue_data,aes(x=Year,y=ratio*4800000,group=1),color="blue",size=2) + geom_line(data=red_data,aes(x=Year,y=ratio*4800000,group=1),color="red",size=2) +scale_y_continuous(sec.axis = sec_axis(~./4800000,name="dupa",labels=scales::percent))+  theme_bw()
#20:44, finalny
...
#testowany wykres
testowane_dane %>% ggplot(aes(przedzialy,Value,fill=Company))+geom_col(position="dodge")+labs(x="Przedziały",y="Ratio",title="Podział współczynników opóźnień w poszczególnych przedziałach czasowych dla firm US Airways(US) i Northwest Airlines(WN) w roku: {closest_state}")+scale_x_discrete(limits=positions)+scale_fill_manual(values = c("#2171b5","#6baed6","#bdd7e7"))+ geom_text(aes(label=sprintf("%.3f",Value),y=round(Value,2)),position=position_dodge(0.9),vjust=-0.8,size=3) + theme(plot.title=element_text(size=13,face="bold")) + transition_states(states=Year,transition_length=2,state_length=1)+ enter_fade()+exit_shrink()+ease_aes('sine-in-out')+ theme_bw() -> dupa

testowane_dane %>% ggplot(aes(przedzialy,Value,fill=Company))+geom_col(position="dodge")+labs(x="Przedziały",y="Ratio",title="Podział współczynników opóźnień w poszczególnych przedziałach czasowych dla firm US Airways(US) i Southwest Airlines(WN) w roku: {closest_state}")+scale_x_discrete(limits=positions)+scale_fill_manual(name="Firma",values = c("#2171b5","#6baed6","#bdd7e7"))+ geom_text(aes(label=sprintf("%.3f",Value),y=round(Value,2)),position=position_dodge(0.9),vjust=-0.8,size=3) + theme(plot.title=element_text(size=13,face="bold")) + transition_states(states=Year,transition_length=2,state_length=1)+ enter_fade()+exit_shrink()+ease_aes('sine-in-out')+ theme_bw() -> dupa
